...
async function runTest(serverUrl, targetId, nuevo, expectedErrorCode) {
    ...
    try {
        ...
        //     console.log("put: el partido recibido no tiene fecha")
        // } else if (!partido.hasOwnProperty('hora')) {
        //     console.log("put: el partido recibido no tiene hora")
        // } else if (!partido.hasOwnProperty('lugar')) {
        //     console.log("put: el partido recibido no tiene lugar")
        } else {
            console.log("put: ok")
            s.separator()
        }
...
async function testPutWithPlayers(serverUrl) {

    const nuevoJugador = {
        id : 3,
        nombre: "Max",
    }
    await runTest(serverUrl, 1, nuevoJugador)
}
